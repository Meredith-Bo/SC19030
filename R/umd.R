#' @title Use Bernstein polynomials to achieve unimodal density estimation using R
#' @name umd
#' @description A density estimation function with unimodal constraint
#' @param data the data to be estimated (numeric)
#' @param bound.type the type of bound (logical)
#' @param fix.lower the minimum of the estimation region (lnumeric)
#' @param fix.upper the maximum of the estimation region (numeric)
#' @param crit methods for selecting the number of weights (logical)
#' @param m the number of weights (numeric)
#' @return a list of weights, optimal number of weights, standard dendity function, lower and upper value
#' @examples
#' \dontrun{
#' data(windspeed)
#' h10<-windspeed$H10
#' umd(h10, bound.type="sd", crit="CN")
#' }
#' @export
library(quadprog)
